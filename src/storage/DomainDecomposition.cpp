   ...
   value. */
    for (int _coord = 0; _coord < 3; ++_coord) {
      ...
      */
      ...
      if (!periodic_[coord] && (local_periodic_[2*coord] ^ local_periodic_[2*coord+1]) && nodeGrid.getGridSize(coord) > 1) {  // a bit special case, TODO move it up
