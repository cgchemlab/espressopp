...
std::set<Particle *> Reaction::postProcess_T1(Particle &p, Particle &partner) {
  ...
  for (PostProcessMap::iterator it = post_process_T1.begin(); it != post_process_T1.end(); ++it) {
    LOG4ESPP_DEBUG(theLogger, "postProcess_T1 " << *this << " pp: " << (*it)->getName() << " p1.type=" << p.type() << " p2.type=" << partner.type());
...
std::set<Particle *> Reaction::postProcess_T2(Particle &p, Particle &partner) {
  ...
  for (PostProcessMap::iterator it = post_process_T2.begin(); it != post_process_T2.end(); ++it) {
    LOG4ESPP_DEBUG(theLogger, "postProcess_T2 " << *this << " pp: " << (*it)->getName() << " p1.type=" << p.type() << " p2.type=" << partner.type());
    ret = (*it)->process(p, partner);
    output.insert(ret.begin(), ret.end());
  }
...
void Reaction::registerPython() {
  ...
  class_<Reaction, shared_ptr<integrator::Reaction> >
      ("integrator_Reaction", init<int, int, int, int, int, int, int, int, shared_ptr<FixedPairList>, real>())
      .def(init<int, int, int, int, int, int, int, int, shared_ptr<FixedPairListLambda>, real>())
...
void DissociationReaction::registerPython() {
  ...
  class_<DissociationReaction, bases<Reaction>, shared_ptr<integrator::DissociationReaction> >
      ...
      .def(init<int, int, int, int, int, int, int, int, real, shared_ptr<FixedPairListLambda>, real>())
      .add_property("type_1",
